...
from wtforms import StringField, PasswordField, SubmitField, SelectField, IntegerField, validators, ValidationError
...
application.secret_key = keys.app_secret()
...
# mongo setup
application.config['MONGO_DBNAME'] = 'safe_db'
...
def settings():
	...
	verify_code = mongo.db.users.find_one({'email': email})['verify_code']
	if verify_code != None:
		return redirect(url_for('verify'))
	return email
